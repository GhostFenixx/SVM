...
class MainSVM {
    ...
    [
     {
      ...
      action: (url, info, sessionID) => {
       ...
       else
       {
        ...
        Logger.error("[SVM] CSM CUSTOM POCKETS - New profile detected, Cancelling function, restart the game to fix it" + e)
        return HttpResponse.nullResponse();
       }
      }
     }], "spt");
    ...
    [
     {
      ...
      action: (url, info, sessionID) => {
       try {
        ...
        Logger.error("[SVM] PMC/SCAV HEALTH - Didn't manage to apply health, new profile?" + e)
        return HttpResponse.nullResponse();
       }
      }
     }], "spt");
 ...
 postDBLoad(container) {
  ...
  if (Config.Loot.EnableLoot) {
   ...
   {
    if(timers != "develop" && timers != "base" && timers != "privatearea" && timers != "terminal" && timers != "factory4_day"  && timers != "hideout" && timers != "laboratory" && timers != "factory4_night" && timers != "town" && timers != "suburbs" )
    {// 3.10 need to rework.
    ...
    for (let AssortR in traders) {
     if (AssortR !== "ragfair" && AssortR !== "638f541a29ffd1183d187f57") {
      for (let level in traders[AssortR].assort.items) {
       ...
       traders[AssortR].assort.items[level].upd["BuyRestrictionMax"] = 0;
       }
      }
     }
    }
